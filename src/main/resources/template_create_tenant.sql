...
CREATE INDEX idxgin_conf ON notify_data USING gin (jsonb jsonb_path_ops);
...
-- COPY notify_data (jsonb) FROM 'data/locales.data' ENCODING 'UTF8';
...
END;
$$ language 'plpgsql';
CREATE TRIGGER set_md_trigger BEFORE INSERT ON myuniversity_mymodule.notify_data
...
END;
$$ language 'plpgsql';
CREATE TRIGGER set_md_json_trigger BEFORE UPDATE ON myuniversity_mymodule.notify_data
